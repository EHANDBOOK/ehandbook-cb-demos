...
flgStandardDocuContent = false;
flgDocuContentFromDocBlocks = true;
...
% 		[filepath,name,ext] = fileparts(excelFile);
	...
	if CreateASCIIDoc == true	    
        ...
        if flgDocuContentFromDocBlocks
            % Generate an overview image for the software component
            writeFunctionOverview(asciidoc_file_id, bdroot);

            % Derive textual contents and embedded screenshots from the
            % Simulink model of the software component and add it to the
            % chapter
            writeModelScreenshotsWithDocBlocks(asciidoc_file_id, bdroot , false);
        end
...
% Adds a function overview for a single function to an AsciiDoc
% file
%
% asciidocFileLocation      Path to directory where the AsciiDoc file shall
% be saved to
% function_name             Name of the function for which a function
% overview shall be created
function writeFunctionOverview(asciidoc_file_id, function_name)
    %fprintf(asciidoc_file_id, '== Function Overview\n\n');
    fprintf(asciidoc_file_id, ['ehbfunctionoverview::', function_name, '[Function Overview for ', function_name, ']\n\n']);
end

% Derive textual contents and embedded screenshots from the
% Simulink model of the software component and add it to the
% chapter
function writeModelScreenshotsWithDocBlocks(asciidoc_file_id, f_name_L, flgRawText)  
    % Find all docblocks in model
    referenced_models = find_mdlrefs(f_name_L,'KeepModelsLoaded',true);
    % referenced_models = flip(referenced_models);
    
    referenced_models_sorted = {};
    [a,b] = size(referenced_models);
    referenced_models_sorted = [referenced_models_sorted; referenced_models(a)];
    for k = 1:size(referenced_models)-1
        referenced_models_sorted = [referenced_models_sorted; referenced_models(k)];
    end
                
    docBlocks_sorted = {};
    for j = 1:size(referenced_models_sorted)
        docBlocks_unsorted = find_system(referenced_models_sorted(j), 'SearchDepth', 15, 'LookUnderMasks', 'on', 'FollowLinks', 'off', 'BlockType', 'SubSystem', 'Mask', 'on', 'MaskType', 'DocBlock');        
        if size(docBlocks_unsorted) > 0
            docBlocks_sorted =  [ docBlocks_sorted; flip(sortSubSys(docBlocks_unsorted)) ];
        end
    end

    docBlocks = docBlocks_sorted;
    
    docBlocksCount = size(docBlocks);
    
    % Iterate over all docblocks
    %for i = docBlocksCount(1):-1:1
    for i = 1:docBlocksCount
        % Get the model level on which the docblock appears
        subsystem_level_name = get_param(docBlocks{i, 1}, 'Parent');
        
        fprintf(asciidoc_file_id, strcat("== ", subsystem_level_name, "\n\n"));
        
        % 1. Create a model screenshot via EHB Model Ref
        %disp(subsystem_level_name);
        subsystem_level_name_without_newline = replace(subsystem_level_name, newline,' ');
        
        ehbmodelref = "ehbmodelref::" + subsystem_level_name_without_newline + "[" + subsystem_level_name_without_newline + "]";
        fprintf(asciidoc_file_id, ehbmodelref);
        fprintf(asciidoc_file_id, '\n');

        % Extract the docblock text
        docBlockUserData = get_param(docBlocks(i), 'UserData');
        docBlockUserText = docBlockUserData{1, 1}.content;
        
        % 2. Embed the docu text of the subsystem
        %disp(docBlockUserText);
        %content = polishDocBlockContent(docBlockUserText);
        content = docBlockUserText;
        ...
        fprintf(asciidoc_file_id, content);
...
% This function is copied from BRP Rotax SoftwareDocumentation scripts
function [ subSysNamesSort ] = sortSubSys( subSysNames )

    %Hinzuf黦en der Position
    for i = 1:size(subSysNames,1)
        pos = get_param(subSysNames{i},'Position');
        subSysNames{i,2} = pos(1);
        subSysNames{i,3} = pos(2);
    end

    %Sortierung von links nach rechts
    subSysNames = sortrows(subSysNames,2);

    %Sortierung bei gleicher Position von oben nach unten
    i = 1;
    while i <= size(subSysNames,1)
        k = i + 1;
        while k <= size(subSysNames,1) && (subSysNames{k,2} - subSysNames{i,2})<= 10
            k = k+1;
        end
        subSysNamesSort(i:(k-1),:) = sortrows(subSysNames(i:(k-1),:),3);
        i = max(k, i+1);
    end
    
    %Delete Position Colums
    subSysNamesSort(:,2:3)=[];
end
