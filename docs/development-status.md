# Development Status & TODOs

This document tracks ongoing tasks, known issues, and future improvements for the EHANDBOOK Container-Build Demos repository.

## Completed Tasks

- ✅ Remove AR_Icons.pptx PowerPoint slides from examples
- ✅ Remove "2022-06 - ETAS EHANDBOOK 10.1 - New approach for generating EHB Containers from .arxml files only.pptx"
- ✅ Turn Demo_ASCET_AddOn_Toolbox.pptx into a demo
- ✅ For any PPTX, only publish the corresponding PDF
- ✅ Reorganized development documentation into `.github/` and `docs/` folders
- ✅ Remove the demo: Demo_EHBCB_DirBased_PDF (PDF to SVG conversion is not a good approach)
- ✅ Do not distribute the TargetLink library (property of dSPACE) - Demo removed

## In Progress / Pending

- [ ] Check if Simulink model of Dir_Based_EHBCB_Simulink_Example is ETAS property and can be released
- [ ] Cross-check and fix AR demos: ASAM_Based_EHBCB and ASAM_Based_EHBCB_Simulink

## Future Improvements

- ✅ Update information for how to clone the repository in README.md (public GitHub)
- [ ] Find a public GitHub repo where to publish the examples
- [ ] Add Code of Conduct (see <https://github.com/etas/edms-examples?tab=coc-ov-file>)
- [ ] Add License (should be ETAS proprietary license, see <https://github.com/etas/edms-examples?tab=License-1-ov-file>)

## Notes

- Please update this document when working on any of these items
- Use checkmarks (✅) for completed tasks and ([ ]) for pending tasks
- Add new items as they are identified
